...
export const LocationProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addLocation = async (loc: LocationData) => {
    try {
      ...
      // Only add to recentLocations if it's NOT a saved address
      if (!loc.saved) {
        const updatedRecent = [loc, ...recentLocations.filter(r => r.address !== loc.address)];
        setRecentLocations(updatedRecent);
        await AsyncStorage.setItem("recentLocations", JSON.stringify(updatedRecent));
        console.log("Recent location added:", loc);
      } else {
        console.log("Saved address selected, not added to recent:", loc);
      }
    } catch (error) {
