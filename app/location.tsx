...
export default function LocationScreen() {
  ...
  // Fetch saved addresses via Redux
  const fetchSavedAddresses = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `http://192.168.0.105:5000/user-addresses/${user.customerId}`
      );
      if (res.data.success) {
        dispatch(setSavedAddresses(res.data.addresses || []));
      } else {
        dispatch(setSavedAddresses([]));
      }
    } catch (err) {
  ...
  const handleSavedAddressClick = (item: any) => {
    addLocation({ address: item.fullAddress || "", lat: item.lat, lng: item.lng });

    router.push({
      pathname: "../map/usedetect",
      params: {
        lat: (item.lat || 0).toString(),
        lng: (item.lng || 0).toString(),
  ...
  const selectPlace = async (placeId: string, description: string) => {
    try {
      ...
      // ✅ Update LocationContext and recent locations
      addLocation({ address: formattedAddress, lat: loc.lat, lng: loc.lng });

      setSearchText("");
      setPlaces([]);

      // Navigate to map with updated location
      router.push({
        pathname: "../map/usedetect",
        params: {
  ...
  const deleteAddress = async (id: string) => {
    try {
      const res = await axios.delete(`http://192.168.0.105:5000/delete-address/${id}`);
      if (res.data.success) {
        const updated = savedAddresses.filter((addr: any) => addr._id !== id);
        ...
        <>
          <View style={styles.buttonRow}>
            <TouchableOpacity
              ...
              onPress={() => router.push("../map/usedetect")}
            >
            ...
            {user && (
              <TouchableOpacity
                ...
                onPress={() => router.push("../map/addnewAddress")}
              >
